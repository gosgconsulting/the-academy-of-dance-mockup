                    ...
                    <div className="relative">
                      <div className="relative h-64 overflow-hidden rounded-xl">
                        {soloImages.map((image, index) => (
                          <img
                            key={index}
                            src={image}
                            alt={`Solo Performance - Image ${index + 1}`}
                            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 cursor-pointer ${
                              index === soloImageIndex ? 'opacity-100' : 'opacity-0'
                            }`}
                            onClick={() => openModal(soloImages, index, 'Solo Performance')}
                          />
                        ))}
                      ...
                      {soloImages.length > 1 && (
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                          {soloImages.map((_, index) => (
                            <button
                              key={index}
                              onClick={() => goToSoloImage(index)}
                              className={`w-2 h-2 rounded-full transition-colors ${
                                index === soloImageIndex 
                                  ? 'bg-white' 
                      ...
                      <div className="relative h-64 overflow-hidden rounded-xl">
                        {groupImages.map((image, index) => (
                          <img
                            key={index}
                            src={image}
                            alt={`Dance Group Performance - Image ${index + 1}`}
                            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 cursor-pointer ${
                              index === groupImageIndex ? 'opacity-100' : 'opacity-0'
                            }`}
                            onClick={() => openModal(groupImages, index, 'Dance Group Performance')}
                          />
                        ))}
                        <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent pointer-events-none"></div>
                      </div>
                      ...
                      {groupImages.length > 1 && (
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                          {groupImages.map((_, index) => (
                            <button
                              key={index}
                              onClick={() => goToGroupImage(index)}
                              className={`w-2 h-2 rounded-full transition-colors ${
                                index === groupImageIndex 
                                  ? 'bg-white' 
                                  : 'bg-white/50 hover:bg-white/70'
                              }`}
                            />
